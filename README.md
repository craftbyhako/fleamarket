...
## 特記事項
- 一部JavaScriptを使用　chat.blade.php　<br>
    【チャット(本文)を入力した状態で他の画面に遷移しても、入力情報を保持できるか】という機能に対し、一部JavaScriptを使用しております。参考UIに「下書き保存」ボタンが無く、サーバーへデータ送信するトリガーの実装が困難であったためです。「その他取引ボタン」のクリック時やフォームの再構成も試しましたが、他の処理が干渉して入力フォームの値をうまく取得できず、保存処理が機能しませんでした。現状では安定して使用できますが、上記の点私の力不足で申し訳ありません。念のため申し添えます。尚、再実装が必要な場合はご指示くださいますよう何卒宜しくお願い致します。
